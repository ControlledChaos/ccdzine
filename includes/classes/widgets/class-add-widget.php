...
class Add_Widget extends \WP_Widget {
	...
	protected function widget_instance() {
		...
		add_action( 'wp_head', function() {
			printf(
				"\n" . __( '<!-- At least on instance of the %s widget is active. -->', 'ccdzine' ) . "\n",
				$this->type_name()
			);
		}, 99 );
	}
	...
	protected function type_name() {
		...
		if ( is_string( $this->type_name ) && ! empty( $this->type_name ) ) {
			$name = __( ucwords( $this->type_name ), 'ccdzine' );

		} else {
			$base = str_replace( [ '-', '_' ], ' ', $this->type_base );
			$name = __( ucwords( $base ), 'ccdzine' );
		}
	...
	protected function type_desc() {
		...
		if ( is_string( $this->type_desc ) && ! empty( $this->type_desc ) ) {
			$type_desc = __( $this->type_desc, 'ccdzine' );
		} else {
			$type_desc = '';
		}
	...
	protected function title_fallback() {
		...
		if ( is_string( $this->title_fallback ) && ! empty( $this->title_fallback ) ) {
			$title_fallback = __( ucwords( $this->title_fallback ), 'ccdzine' );
		} else {
	...
	public function form( $instance ) {
		...
		// Look for the file.
		$file = CCD_PATH . $this->widget_views() . $this->form_file();
	...
	public function widget( $args, $instance ) {
		...
		// Look for the file.
		$file = CCD_PATH . $this->widget_views() . $this->widget_file();
