...
class Editor_Options {
	...
	public static function register_settings() {
		...
		$heading_default = __( 'Default editor for all users', 'ccdzine' );
		$heading_allow   = __( 'Allow users to switch editors', 'ccdzine' );
	...
	public static function editor_settings_default() {
		include_once CCD_PATH . 'views/backend/forms/partials/settings-writing-editor-default.php';
	}
	...
	public static function editor_settings_allow() {
		include_once CCD_PATH . 'views/backend/forms/partials/settings-writing-editor-allow.php';
	}
	...
	public static function user_settings() {
		...
		include_once CCD_PATH . 'views/backend/forms/partials/settings-user-editor-default.php';
	}
	...
	public static function network_settings() {
		include_once CCD_PATH . 'views/backend/forms/partials/settings-network-editor-default.php';
	}
	...
	public static function do_meta_box( $post ) {
		...
		<p style="margin: 1em 0;">
			<a href="<?php echo esc_url( $edit_url ); ?>"><?php _e( 'Switch to block editor ', 'ccdzine' ); ?></a>
		</p>
		<?php
	}
	...
	public static function enqueue_block_editor_scripts() {
		...
		wp_enqueue_script(
			'editor-options',
			CCD_URL . 'assets/js/editor-options' . $suffix . '.js',
		...
		wp_localize_script(
			'editor-options',
			'editorOptionsL10n',
			[ 'linkText' => __( 'Switch to rich text editor ', 'ccdzine' ) ]
		);
	}
	...
	public static function add_edit_links( $actions, $post ) {
		...
		$text       = _x( 'Edit Blocks', 'Editor Name', 'ccdzine' );
		$label      = sprintf( __( 'Edit &#8220;%s&#8221; in the block editor', 'ccdzine' ), $title );
		...
		$text         = _x( 'Edit Rich Text', 'Editor Name', 'ccdzine' );
		$label        = sprintf( __( 'Edit &#8220;%s&#8221; in the rich text editor', 'ccdzine' ), $title );
	...
	public static function add_post_state( $post_states, $post ) {
		...
		if ( ! $editors['editor_options'] && ! $editors['block_editor'] ) {
			...
			$state = '<span class="editor-options-forced-state">' . _x( 'rich text editor', 'Editor Name', 'ccdzine' ) . '</span>';

		} elseif ( ! $editors['editor_options'] && $editors['block_editor'] ) {

			// Forced to block editor.
			$state = '<span class="editor-options-forced-state">' . _x( 'block editor', 'Editor Name', 'ccdzine' ) . '</span>';
			...
			$state = $is_tinymce ? _x( 'Rich Text', 'Editor Name', 'ccdzine' ) : _x( 'Blocks', 'Editor Name', 'ccdzine' );
		}
